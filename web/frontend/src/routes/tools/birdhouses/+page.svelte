<script lang="ts">
    ...
    interface ProTipsData {
        calculation_methodology: {
            ...
            base_formula: string;
            data_points: Array<{log_type: string, hunter_xp: number, note: string}>;
        };
        game_mechanics: {
            run_duration: string;
            setup_time: string;
    ...
    async function loadProTips() {
        ...
        try {
            const response = await fetch('http://localhost:8080/api/tools/birdhouse/tips');
            if (!response.ok) {
    ...
    <div bind:this={resultsElement}>
        ...
        {#if currentApiResult}
            <div class="animate-slide-up">
                <BirdhouseResults apiResult={currentApiResult} iconSrc={toolConfig.iconSrc} />
            </div>
        {/if}
    </div>
            ...
            >
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-theme-text-secondary">Learn how Birdhouse XP rates and rewards are calculated</p>
                    </div>
                </div>
                <svg 
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                                    {#each proTipsData.calculation_methodology.data_points as point}
                                        <div class="flex justify-between items-center bg-theme-bg-secondary/50 rounded px-3 py-2">
                                            <span class="text-sm text-theme-text-secondary">{point.log_type} logs</span>
                                            <span class="text-sm font-medium text-theme-text-primary">{point.hunter_xp} XP</span>
                                        </div>
                                    {/each}
                                </div>
                            </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <p class="text-sm text-theme-text-secondary mb-1">
                                            <strong>Run Duration:</strong> {proTipsData.game_mechanics.run_duration}
                                        </p>
                                        <p class="text-sm text-theme-text-secondary">
                                            <strong>Setup Time:</strong> {proTipsData.game_mechanics.setup_time}
                                        </p>
                                    </div>
                                    <div>
                        ...
                        <div>
                            ...
                            <div class="bg-theme-bg-primary/50 rounded-lg p-4">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <p class="text-sm text-theme-text-secondary mb-2">
                                            <strong>Nest Rates:</strong> {proTipsData.reward_calculation.nest_rates}
                                        </p>
                                        <p class="text-sm text-theme-text-secondary">
                                            <strong>Seed Variety:</strong> {proTipsData.reward_calculation.seed_variety}
                                        </p>
                                    </div>
                                    <div>
                                        <p class="text-sm text-theme-text-secondary mb-2">
