<script lang="ts">
    ...
    import GOTRForm from '$lib/components/gotr/InputForm.svelte';
    import GOTRResults from '$lib/components/gotr/ResultsDisplay.svelte';
    ...
    interface ProTipsData {
        ...
        reward_calculation: {
            searches_per_game: string;
            search_variation: string;
            loot_simulation: string;
            gp_per_hour: string;
        };
    }
    ...
    async function handleCalculation(event: CustomEvent<{ resultData: GOTRApiResult }>) {
    ...
    async function loadProTips() {
        ...
        try {
            const response = await fetch('http://localhost:8080/api/tools/gotr/tips');
            if (!response.ok) {
...
<div class="max-w-6xl mx-auto">
    ...
    <header class="mb-10 text-center">
        <div class="flex items-center justify-center mb-4">
            <div class="w-16 h-16 bg-gradient-to-r from-purple-500 to-blue-600 rounded-xl flex items-center justify-center p-2 mr-4">
    ...
    <div class="max-w-2xl mx-auto mb-8">
        <div class="bg-theme-bg-secondary rounded-card border border-theme-border-accent/20 p-6 shadow-card">
            <GOTRForm on:calculated={handleCalculation} on:error={handleCalcError} />
        </div>
    </div>
    ...
    <div bind:this={resultsElement}>
        ...
        {#if currentApiResult}
            <div class="animate-slide-up">
                <GOTRResults apiResult={currentApiResult} iconSrc={toolConfig.iconSrc} />
            </div>
        {/if}
    </div>
    ...
    <div class="max-w-4xl mx-auto mt-12">
        <div class="bg-theme-bg-secondary rounded-card border border-theme-border-accent/20 shadow-card overflow-hidden">
            <button 
                ...
                class="w-full px-6 py-4 text-left bg-gradient-to-r from-blue-500/10 via-purple-500/5 to-transparent hover:from-blue-500/15 hover:via-purple-500/10 transition-all duration-200 flex items-center justify-between group"
            >
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-theme-text-secondary">Learn how GOTR XP rates and rewards are calculated</p>
                    </div>
                </div>
                <svg 
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <p class="text-sm text-theme-text-secondary mb-1">
                                            <strong>Game Duration:</strong> {proTipsData.game_mechanics.game_duration}
                                        </p>
                                        <p class="text-sm text-theme-text-secondary">
                                            <strong>Games per Hour:</strong> {proTipsData.game_mechanics.games_per_hour}
                                        </p>
                                    </div>
                                    <div>
                        ...
                        <div>
                            ...
                            <div class="bg-theme-bg-primary/50 rounded-lg p-4">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <p class="text-sm text-theme-text-secondary mb-2">
                                            <strong>Searches per Game:</strong> {proTipsData.reward_calculation.searches_per_game}
                                        </p>
                                        <p class="text-sm text-theme-text-secondary">
                                            <strong>Search Variation:</strong> {proTipsData.reward_calculation.search_variation}
                                        </p>
                                    </div>
                                    <div>
                                        <p class="text-sm text-theme-text-secondary mb-2">
                                            <strong>Loot Simulation:</strong> {proTipsData.reward_calculation.loot_simulation}
                                        </p>
                                        <p class="text-sm text-theme-text-secondary">
